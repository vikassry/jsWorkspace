	...
	the_point_on: function(ob) {
		return function(point){
			if(typeof ob.radius=='number'){
				if(Math.sqrt(Math.pow(ob.centre.x-point.x,2)+Math.pow(ob.centre.y-point.y,2))==ob.radius)
					return true;
			}
...
r.Line=function(a,b){
	...
	line_ob.hasPoint=function(p){
		...
		var d2=Math.sqrt(Math.pow(p.x-line_ob.end.x, 2)+Math.pow(p.y-line_ob.end.y, 2));
		return (line_ob.length==(d1+d2));
	};
...
r.if = function(boolean_val){
	return {then:function(funref){
		return {else:function(arg){
			if(boolean_val==true)
				return funref();
			if(boolean_val==false && typeof(arg)=='string')
				return arg;
			if(boolean_val==false && typeof(arg)=='function')
				return arg();
		}
	}
}
